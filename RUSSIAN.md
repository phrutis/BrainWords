...
У кого отваливается карта от пула.<br>
Скачайте батник для Windows.<br>
https://github.com/phrutis/BrainWords/releases/download/v4.0/START_POOL.bat

Нужно открыть батник в текстовом редакторе (Emeditor или Notepad++) зайти в поиск, выбрать заменить Vasya на ваш ник. <br>
Нажать кнопку заменить все. Сохранить батник. <br>
Разместить файл START_POOL.bat рядом с программой bb.exe<br>
Запустить START_POOL.bat. Должно открыться 2 окна.<br>
В первом окне таймер (время до перезагрузки), во втором окне клиент пула.<br>
Первый перезапуск программы будет через 5 минут.<br>
После перезагрузки окно клиента открывается свернуто, смотрите его в низу экрана, его можно развернуть.<br>
Все последующие перезапуски через 2 часа. (всего 30 перезагрузок расчитан на 58 часов)<br>
Рекомендую перезапускать батник через 2 дня. Закрыли таймер, запустили батник снова.<br>
Можете изменить время до перезагрузки для каждого цикла. 7200 - это секунды.<br>
Укажите свои значения в секундах.<br>
<hr>
