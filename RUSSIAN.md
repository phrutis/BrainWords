# BrainWords GPU Pool

## Брут паролей и фраз от мозговых кошельков
![Brain_Wallet](https://github.com/phrutis/BrainWords/assets/140947743/f55f6027-e737-4f77-b156-2a0dd170e366)

🚀 Это самым быстрый GPU пул в мире. 🚀<br>
В программе используются быстрые алгоритмы и библиотеки CUDA.<br>
Нам удалось достичь скорости RTX 4090 = 360 М/с (36000000 парольных фраз/сек)<br>
Сейчас ищем 23353984 адреса BTC Legacy 1...<br>
Программа ищет сразу в двух режимах.<br>
Из одного приватного ключа проверяются 2 адреса (compressed и uncompressed)

## Скорость поиска

| GPUs card     | Скорость|
|---------------|---------|
| RTX 4090      | 360 M/s |
| RTX 3090      | 170 M/s |
| RTX 3070      | 100 M/s |
| RTX 3050      | 45 M/s |
| RTX 2060      | 30 M/s |

*M (миллионов парольных фраз в сек)

## Условия выплат 💰
**50%** от баланса от находки выплачиваются тому кто нашел пароль (фразу).<br>
**30%** от баланса получают организаторы пула.<br>
**20%** от баланса делится на всех охотников (см % от пула), **кроме победителя**.

**Находки до $100 выплачиваются тому кто её нашел!** 

Если вы готовы искать вместе с нами 24/7, присоединяйтесь.<br>
Telegram: https://t.me/+k1jKsx_fxTo1MzQx<br>
Онлайн статистика пула: [https://phrutisbrainwords.com](https://phrutisbrainwords.com)

## Запуск программы
### Ubuntu Client
Terminal<br>
```mkdir bb```<br>
```cd bb```<br>128 бит это как на велосипеде на луну лететь. 

Браины хоть и вычистили, всё же имеет больше перспектив найти.
Так же в браинах есть преимущества.

0.Поиск мозговых кошельков. В отличии от других пулов были находки. Значит будут ещё, есть шанс найти заначку (холодный кошелек)
Пик создания браинов лет 12-15 назад. Тогда за 50 BTC не купишь ничего, да и потратить некуда.
Копили кучами, так как стоимость низкая тупо забывали, выбрасывали. Переустановил windows, потерял флешку, удалил.
Браин создавались с целью хранить пароль или фразу в голове. Кто помнит свой пароль из 2012-го года? 
Пример тому Minikeys Casascius 4000 сувенирных монет в мусорке.
700 у коллекционеров.
90% даже не знали что под галограммой пароль.

1. Поиск идёт с полной базой btc адресов.
Так как каждый приватный ключ на выходе рандомный. Есть шанс найти любой адрес из базы.
Рандом 100% валидный. 
Рандом 100%-тный, каждый ключ рандом.

2.Программа ищет сразу адреса 1, 3, bc.
Для адресов 1.. работает в двух режимах (сжатый, не сжатый)
Это увеличивает шанс в два раза. В теории можно найти сжатый адрес в публичным ключей 04...., так же и наоборот.
Можно найти с публичным ключей 02.... не сжатый адрес сатоши от 2009 года.

3.Одному адресу в 256-ти битах соответствуют 
79,228,162,514,264,337,593,543,950,336 приватных ключей
Так как поиск идёт в двух режимах 
Одному адресу из базы адресов соответствуют 
158,456,325,028,528,675,187,087,900,672 приватных ключей в 257-ми битном диапазоне .
Это среднее значение. Приватных ключей которые соотаетствуют одному адресу может быть меньше или больше.
Для тех кто не понимает о чем выше.

У вас есть 1000 ключей от 5-ти замков. Каждый ключ подходит только к одному замку.
1000/5 = 200 ключей (среднее значение)
На практике может быть так
Замок 1 = 109 ключей
Замок 2 = 315 ключей
Замок 3 = 237 ключей
Замок 4 = 186 ключей
Замок 5 = 163 ключей

158,456,325,028,528,675,187,087,900,672 x 20000000 адресов в базе = 
Эквивалент кол-во адресов в блум фильтре.

Скорость программы RTX 4090 = 360 млн. ключей в сек.
Для примера то что 4090 проходит за сутки, brainflayer за всю свою историю.
Это не означает что поиски будут лёгкие и монеты будут сыпаться.
Здесь тоже нужна удача и терпение найти что нибудь крупное. 
У кого есть вопросы, идеи, предложения пишите обсудим.
```wget https://phrutisbrainwords.com/downloads/bb```<br>
```chmod +x bb```<br>
```./bb -u YourNickName```

### Vast 4090/3090/3080/3070
IMAGE cuda:12.0.1-devel-ubuntu20.04 (Use Jupyter Lab interface)<br>
Use Jupyter Lab interface<br>
Terminal<br>
```mkdir bb```<br>
```cd bb```<br>
```wget https://phrutisbrainwords.com/downloads/bb```<br>
```chmod +x bb```<br>
```./bb -u YourNickName```

### Windows
Распаковать zip папку BBWindows.zip<br>
Откройте командную строку и введите:<br>
Run: ```bb.exe -u YourNickName```

### HIVEOS 
(This will be easier in the future)<br>
```sudo mkdir bb && cd bb && sudo apt install aria2 && wget https://phrutisbrainwords.com/downloads/bb && chmod +x bb && ./bb -u YourNickName```

### Часто задаваемые вопросы ❓

**Что такое браин валет?**
<br><br>
Это пароли или фразы преобразованные в sha256 на выходе получаем приватный ключ от адреса.<br>
На адресе люди хранили монеты а парольную фразу в голове.<br>
Такой метод использовался 10-15 лет назад. В то время монеты почти ничего не стоили.<br>
Пароли терялись, забывались. Вы помните свой пароль 12 лет назад? Уверены?<br>
Один символ, точка или регистр не так и всё кошелек потерян. Кнопки восстановить не существует.<br>
Поищите больше информации о brain wallet в интернете.<br>
Наглядный генератор браин валетов [https://brainwalletx.github.io](https://brainwalletx.github.io)
<hr>

**Как выглядат браин валеты?**
<br>
Были ли вообще находки?
<br><br>
Вот вам белый список паролей, фраз, балансов изучайте<br>
https://privatekeyfinder.io/brainwallet/bitcoin/
<hr>

**Я первый раз подключился. Программа зависла прошло уже 15 минут**
<br><br>
При первом подключении клиента скачивается база BTC адресов<br>
Обычно это длится от 3 до 10 минут<br>
Это зависит от скорости интернет. Может доходить до 20 мин.<br>
<hr>

**Я первый раз подключился. Программа зависла на этом месте, прошло уже 20 минут**
<br>
**Generating the EC point cache. This is only done once...***
<br><br>
При первом запуске генерируется таблица ускорения.<br>
Обычно это длится от 5 до 12 минут<br>
Если у вас много карт и слабый процессор это может занять больше времени.<br>
Дождитесь завершения генерации таблицы.<br>
По окончанию генерации таблица сохранится в файл ecmult_big-123456788-24.bin 1-6 ГБ. (не удаляйте его)<br>
При повторном запуске таблица не будет генерироваться, клиент запустится сразу.<br>
<hr>

**Я первый раз подключился. Программа зависла прошло 25 минут???**
<br>
У меня написало: loaded addresses: 23353984 и зависоло! НЕТ СКОРОСТИ!
<br><br>
Закройте окно. Откройте и запустите клиент заново:<br>
./bb -u BTC-ADDRESS<br>
Клиент начнет работать и вы увидите скорость<br>
<hr>

**У вас уже были находки в браинах?**
<br><br>
Да были, 20-30 находок от $2 до $1270.<br>
Наши новые находки вы можете посмотреть на сайте статистики https://phrutisbrainwords.com
<hr>

**Я нашел где моя выплата? Прошло уже 8-10 часов!**
<br><br>
Найденный приватный ключ импортируется в BitCoin Core.<br>
Если адрес сжатый 2018-2020 годов требуется 1-3 часа что бы импортировать его завершить индексацию.<br>
Мы используем очень быстрый SSD M.2, но если адрес 2009-2013 годов то он не сжатый, нам понадобится от 6 до 9 часов индексации.<br>
Вам нужно понимать что в BitCoin Core может индексироваться предыдущая находка.<br>
Так же нужно учитывать часовые пояса и другие факторы<br>
<hr>

**У меня не запускается программа, ошибка..**
<br><br>
Скачайте самые новые (последние) драйвера для вашей видеокарты.<br>
Если проблема не исчезла напишите об этом в чате.<br>
Телепатов в группе нет, сделайте скриншот окна программы с ошибкой.
<hr>

**Как сделать скриншот?**
<br><br>
Windows Нажмите Ctrl + PrtSc, откройте Point Ctrl + v обрезать и отправить в группу.
<hr>

**Как узнать что именно я нашел? парольную фразу, баланс?**
<br><br>
В телеграм группе бот **BrainMessage** пришлет 2 уведомления:<br>
**Vasya Found Something, Please wait for Details**<br>
Это говорит о том что пользователь **Vasya** нашел парольную фразу.<br>
Пользователю Vasya нужно посмотреть в окно своего клиента. В окне будет найденный им BTC адрес.<br>
Проверить баланс найденного адреса можно здесь https://www.blockchain.com/ru/ или в любом другом блокчейне.<br>
Так он будет знать баланс вашей находки, ему полагается 50%<br>

Админы так же получают сообщение от бота с находкой. Они знают кто нашел, ключ и другую информацию.<br> 
Если сумма крупная делается скриншот статистики что бы зафиксировать % от пула на моменет находки.<br>
Админы разместят адрес находки, скриншот статистики в чате группы.<br>
После всех выплат админы разместят парольную фразу, приватный ключ в чате.<br>

Тот кто нашел может самостоятельно разместить найденный адрес из клиента в чате. Или просто баланс находки.<br>
В этом случае его анонимность будет раскрыта.<br>

На сайте https://phrutisbrainwords.com будет мигать жёлтым цветом ⭐<br>
Пример находки в клиенте:<br>
![Screenshot_20240109_221139_AnyDesk](https://github.com/phrutis/BrainWords/assets/140947743/1f0eebf8-3e19-45aa-8938-b788b30bfe72)
<hr>

**Почему бот присылает сразу 2 сообщения о находке?**
<br>
Я нашел 2 находки сразу сжатый, не сжатый адрес?
<br><br>
Тогда бот пришлет 4 сообщения.<br>
Бот так же отправляет сообщение о находке админам.<br>
По этому в общий чат приходит 2 сообщения.<br>
<hr>

**Я арендую карты у Александра, Дмитрия вопрос:**
<br>
Как мне обезопасить себя что бы они не получили мою находку (% от пула)?<br>
Ведь они увидят найденный адрес и могут не получить свою долю?<br>
<br><br>
Когда будете арендовать риги (карты) у майнеров попросите их указать ваш BTC адрес.<br>
В этом случае выплата пойдет на адрес который указан.<br>
<hr>

**Я майнер не хочу светится в чате и т.п. Как мне участвовать анонимно?**
<br><br>
Запустите клиент, укажите ваш пустой BTC адрес.
<hr>

**Как мне рассчитать свой % от пула?**
<br>
Как делится находка?
<br><br>
Вот вам наглядный пример распределения находки:<br>
12EWQG73gVgsEmyMy73ZS2FjHVPpCbGDd7 нашел адрес $150000<br>
Делается скриншот статистики и отправляется в чат для фиксации % от пула.<br>
12EWQG73gVgsEmyMy73ZS2FjHVPpCbGDd7 получает $75000 (50%)<br>
Админы получают $45000 (30%)<br>
20% от пула - это $30000 (100%)<br>
К примеру ваш % в таблице статистики составляет 4.73972%<br>
(% высчитывается на основе кол-ва пройденных комбинаций (тотал)<br>
Вы получите $1421, это покроет часть ваших расходов на электричество и другое<br>
Победитель 12EWQG73gVgsEmyMy73ZS2FjHVPpCbGDd7 не получает % от пула! Его % от пула идёт на оплату коммисий транзакций выплат.<br>
Если после всех выплат у него останутся монеты, они снова будет разделены между охотниками.
<hr>

**Как указать определённые карты?**
<br><br>
Добавьте в строку запуска -d 0,5,6,7<br>
Список карт начинается от 0.<br>
Укажите нужные карты через запятую
<hr>

**Я не нашел ответ на свой вопрос**
<br><br>
Задайте его в Telegram чате: https://t.me/+k1jKsx_fxTo1MzQx
<br><br>
